...
import { addLauncherItems, CreateNavCommand, INavCommandOptions } from '../commands';
...
describe('Fornax Commands', () => {
  ...
  describe('addLauncherItems', () => {
    ...
    it('should call launcher.add three times', () => {
      ...
      expect(mockAdd).toHaveBeenCalledWith(expect.objectContaining({ command: 'fornax:dashboard' }));
      expect(mockAdd).toHaveBeenCalledWith(expect.objectContaining({ command: 'fornax:gh-docs' }));
      expect(mockAdd).toHaveBeenCalledWith(expect.objectContaining({ command: 'fornax:discourse' }));
    });
  });
  ...
  describe('CreateNavCommand', () => {
    ...
    it('should add command to app, add item to palette, and return command ID', () => {
      ...
      expect(mockAddCommand).toHaveBeenCalledWith(baseNavOptions.id, expect.objectContaining({
        label: baseNavOptions.label,
        icon: baseNavOptions.icon,
      }));
      expect(mockPaletteAddItem).toHaveBeenCalledTimes(1);
      expect(mockPaletteAddItem).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('command execution logic', () => {
      it('should open new window via window.open if diag_body is null', async () => {
        const options: INavCommandOptions = { ...baseNavOptions, diag_body: null, navlink: 'http://newtab.com' };
        CreateNavCommand(category, options, mockPalette as ICommandPalette, mockApp as JupyterFrontEnd);
      ...
      it('should show dialog and navigate current page on "accept" if diag_body is provided', async () => {
        ...
        mockShowDialog.mockResolvedValue({ button: { accept: true, label: 'Yes' } } as Dialog.IResult<any>);
      ...
      it('should show dialog and NOT navigate current page on "cancel" if diag_body is provided', async () => {
        const options: INavCommandOptions = { ...baseNavOptions, diag_body: 'Are you sure?' };
        mockShowDialog.mockResolvedValue({ button: { accept: false, label: 'Cancel' } } as Dialog.IResult<any> );
